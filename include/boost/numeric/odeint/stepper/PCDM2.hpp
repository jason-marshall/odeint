...
#ifndef BOOST_NUMERIC_ODEINT_STEPPER_PCDM2_HPP_DEFINED
#define BOOST_NUMERIC_ODEINT_STEPPER_PCDM2_HPP_DEFINED
...
namespace boost {
  namespace numeric {
	...
	class Rotation = Coor ,
	class AngularVelocity = Coor ,
	class AngularAcceleration = Coor ,
	class Time = Value ,
	class TimeSq = Time ,
	...
	>
      class PCDM2 : public algebra_stepper_base< Algebra , Operations >
      {
      public:
	...
	typedef AngularVelocity angular_velocity_type;
	typedef AngularAcceleration angular_acceleration_type;
	typedef std::pair< coor_type , velocity_type > translation_state_type;
	typedef std::pair< velocity_type , acceleration_type > translation_deriv_type;
	typedef std::pair< rotation_type , angular_velocity_type > rotation_state_type;
	typedef std::pair< angular_velocity_type , angular_acceleration_type > rotation_deriv_type;
	typedef state_wrapper< acceleration_type > wrapped_acceleration_type;
	typedef state_wrapper< angular_acceleration_type > wrapped_angular_acceleration_type;
	...
	static const order_type order_value = 2;
	...
	// constructor, need to update
	PCDM2( const algebra_type & algebra = algebra_type() )
	  : algebra_stepper_base_type( algebra ) , m_first_call( true )
	  , m_a1() , m_a2() , m_current_a1( true ) { }

	// step, should be good
	template< class System , class StateInOut >
	void do_step( System system , StateInOut & translation , StateInOut & rotation , time_type t , time_type dt )
	{
	  do_step_v1( system , translation , rotation , t , dt );
	}

	// step, should be good    
	template< class System , class StateInOut >
	void do_step( System system , const StateInOut & translation , const StateInOut & rotation , time_type t , time_type dt )
	{
	  do_step_v1( system , translation , rotation, t , dt );
	}

	// step, call should be good, need to write algorithm    
	template< class System , class CoorIn , class VelocityIn , class AccelerationIn ,
		  ...
		  class RotationIn , class AngularVelocityIn , class AngularAccelerationIn ,
		  class RotationOut , class AngularVelocityOut , class AngularAccelerationOut >
	void do_step( System system , CoorIn const & position_in , VelocityIn const & velocity_in , AccelerationIn const & acceleration_in ,
		      CoorOut & position_out , VelocityOut & velocity_out , AccelerationOut & acceleration_out ,
		      RotationIn const & rotation_in , AngularVelocityIn const & angular_velocity_in , AngularAccelerationIn const & angular_acceleration_in ,
		      RotationOut & rotation_out , AngularVelocityOut & angular_velocity_out , AngularAccelerationOut & angular_acceleration_out , time_type t , time_type dt )
	{

	  //
	  // NEED TO WRITE THE ALGORITHM HERE
	  //
	...
	// need to rewrite everything below here....
...
#endif // BOOST_NUMERIC_ODEINT_STEPPER_PCDM2_HPP_DEFINED
